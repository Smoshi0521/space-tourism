...

      {
        spaceData.length !== 0 && (
          <div className='flex items-center justify-center tablet:justify-start gap-8 relative tablet:pl-10 desktop:px-[185px] tablet:mt-10'>
            <p className='text-[#FFFFFF]/25 font-barlow font-bold text-xl desktop:text-[28px]'>01</p>
            <h2 className='font-barlow text-xl tracking-widest desktop:text-[28px]'>PICK YOUR DESTINATION</h2>
          </div>
        )
      }
              ...
              <motion.div
                ...
                <div className=' w-full flex justify-center relative desktop:w-fit'>
                  <motion.img
                    ...
                    src={spaceData[destination]?.images?.png} className={`moonSize`} />
                </div>

                <div className='w-full flex flex-col items-center gap-5 desktop:max-w-[445px] '>

                  <div className='flex flex-row w-full justify-center gap-5 tablet:gap-10 relative desktop:justify-start'>
                    {spaceData.length !== 0 && (
                      spaceData?.map((desti: any, index: number) => (
                        <button
                          ...
                          <p className='text-paleblue font-barlow text-lg tablet:text-xl tracking-[2.7px]'>{desti?.name}</p>
                          <AnimatePresence>
                            {
                  ...
                  <div className='flex flex-col items-center w-full tablet:max-w-[500px] desktop:max-w-full gap-10'>
                    ...
                    <div className='w-full flex flex-col items-center desktop:items-start'>
                      ...
                      <p className='text-paleblue text-center text-lg desktop:text-left desktop:text-[18px]'>{spaceData[destination]?.description}</p>
                    </div>
                  ...
                  <div className='flex flex-col items-center tablet:flex-row tablet:justify-evenly gap-5 desktop:justify-start desktop:gap-20 w-full tablet:max-w-[500px] mt-2 desktop:max-w-full'>
                    <div className='text-center desktop:text-left space-y-2'>
                      <p className='text-paleblue text-[18px] tracking-wider'>AVG.DISTANCE</p>
                      <p className='font-bellafair text-[28px]'>{spaceData[destination]?.distance.toUpperCase()}</p>
                    </div>
                    <div className='text-center desktop:text-left'>
                      <p className='text-paleblue text-[18px] tracking-wider'>EST.TRAVEL TIME</p>
                      <p className='font-bellafair text-[28px]'>{spaceData[destination]?.travel.toUpperCase()}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
